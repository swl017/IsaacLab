...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            segmentation_data = env.scene["tiled_camera"].data.output["semantic_segmentation"][1].cpu().numpy()
            # Extract bounding boxes if bbox extractor is enabled
            bbox_extractor = BBoxExtractor(
                min_area=0,
                class_filter=[1],
            )
            # bboxes = extract_bboxes_from_camera(env.scene["camera"].data.output, bbox_type="semantic", min_area=0, class_filter=None)
            ...
            if image_display is None:
                ...
                #     image=env.scene["camera"].data.output["rgb"][1].cpu().numpy(), bboxes=bboxes, bbox_type="semantic"))
