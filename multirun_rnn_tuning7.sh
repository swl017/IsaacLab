...
EXP_DIR="iris_ma4_mappo_rnn_tuning7_$(date +%Y%m%d_%H%M%S)"

export IDE_DEBUG_MODE=False
        ...
        for entropy in "${ENTROPY_SCALES[@]}"; do
            for mini_batch in "${MINI_BATCHES[@]}"; do
                for rollout in "${ROLLOUTS[@]}"; do
                    ...
                    # Skip first configuration (already completed)
                    if [[ $current -eq 1 ]]; then
                        echo "[$current/$total] Skipping first configuration (already completed)"
                        skipped=$((skipped + 1))
                        continue
                    fi
