...
class IrisEnvCfg(DirectRLEnvCfg):
    ...
    stabilize_reward_scale = -0.01
    yaw_reward_scale = -0.1
    yaw_rate_scale = 0.01
    yaw_rate_reward_scale = ang_vel_reward_scale * yaw_rate_scale
    target_speed = 5.0
...
class IrisEnv(DirectRLEnv):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # self._thrust[:, 0, :2] = self._robot_weight * self._actions[:, :2]
        self._thrust[:, 0, 2] = self.cfg.thrust_to_weight * self._robot_weight * (self._actions[:, 0] + 1.0) / 2.0
        self._moment[:, 0, :2] = self.cfg.moment_scale * self._actions[:, 1:-1]
        self._moment[:, 0, 2] = self.cfg.yaw_moment_scale * self._actions[:, -1]
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        ang_scale = torch.ones_like(self._robot.data.root_ang_vel_b)
        ang_scale[:, 2] = self.cfg.yaw_rate_scale
        ang_vel = torch.sum(torch.square(self._robot.data.root_ang_vel_b * ang_scale), dim=1)
