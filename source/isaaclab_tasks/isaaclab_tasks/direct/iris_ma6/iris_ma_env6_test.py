...
class IrisMA6TestEnv(DirectMARLEnv):
    ...
    Observation space per agent: 13D
    ...
    def __init__(self, cfg: IrisMA6TestEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        for agent_id in cfg.possible_agents:
            robot = self._robots[agent_id]
            # Get mass from physics
            mass = robot.root_physx_view.get_masses().mean().item()
    ...
    def _pre_physics_step(self, actions: Dict[str, torch.Tensor]):
        ...
        for idx, agent_id in enumerate(self.cfg.possible_agents):
            ...
            self.cmd_vel[:, idx, 4] = action[:, 4] * self.cfg.max_gimbal_rate
            self.cmd_vel[:, idx, 5] = action[:, 5] * self.cfg.max_gimbal_rate
    ...
    def _get_observations(self) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            Dictionary mapping agent_id to observation tensor (N, 13).
        """
        obs = {}
        for idx, agent_id in enumerate(self.cfg.possible_agents):
            robot = self._robots[agent_id]
            ...
            # Concatenate observation: pos(3) + vel(3) + quat(4) + gimbal_yaw(1) + gimbal_pitch(1) + zoom(1)
