# file: bbox_raycaster_v2_cfg.py
"""Configuration for the self-contained batched bbox raycaster V2."""
...
@configclass
class BBoxRayCasterV2Cfg:
    """Configuration for the batched bounding box raycaster V2.
    ...
    """Whether the observing agent's own body can occlude the camera line of sight."""

    self_occlusion_min_hit_distance_m: float = 0.05
    """Ignore self-hits closer than this distance to avoid camera-mount false positives."""

    enable_inter_target_occlusion: bool = True
    """Whether to apply image-space inter-target occlusion after geometric detection."""

    inter_target_iou_soft_threshold: float = 0.10
    """IoU threshold above which soft confidence decay begins."""

    inter_target_iou_hard_threshold: float = 0.30
    """IoU threshold above which the farther target becomes empty."""
