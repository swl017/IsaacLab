...
Adapted from iris_ma5 to iris_ma6's 7D action space with gimbal tracking.
"""
...
class GreedyEquiangularPolicy:
    ...
    Additionally, gimbal yaw/pitch are driven to point at the target.

    The policy outputs normalized actions in [-1, 1] compatible with the
    iris_ma6 environment action space: [vx, vy, vz, yaw_rate,
    ...
    def compute_actions(
        ...
        Args:
            ...
            agent_orientations: {agent_id: [N, 4]} current agent quaternions (wxyz).
        ...
        for idx, (agent_id, pos) in enumerate(agent_positions.items()):
            ...
            # Gimbal control: point at target
            to_target = target_position - pos  # [N, 3]
            to_target_xy = to_target[:, :2]
            to_target_dist_xy = to_target_xy.norm(dim=-1, keepdim=True).clamp(min=1e-6)

            # Desired gimbal yaw: azimuth to target (relative to body forward)
            # Simplified: just use world-frame azimuth error as rate command
            gimbal_yaw_rate = torch.atan2(to_target[:, 1], to_target[:, 0]) * self.gimbal_gain
            gimbal_yaw_rate = (gimbal_yaw_rate / math.radians(360.0)).clamp(-1.0, 1.0)

            # Desired gimbal pitch: elevation to target
            gimbal_pitch_rate = torch.atan2(-to_target[:, 2], to_target_dist_xy.squeeze(-1)) * self.gimbal_gain
            gimbal_pitch_rate = (gimbal_pitch_rate / math.radians(360.0)).clamp(-1.0, 1.0)
            ...
            action[:, 0:3] = vel_cmd
            # yaw_rate stays 0
            action[:, 4] = gimbal_yaw_rate
            action[:, 5] = gimbal_pitch_rate
            # zoom_rate stays 0
