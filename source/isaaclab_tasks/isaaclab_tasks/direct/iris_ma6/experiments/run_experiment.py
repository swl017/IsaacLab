...
"""Run a single named experiment from the iris_ma6 registry.

Usage:
    # Run with default seed
    ./isaaclab.sh -p source/isaaclab_tasks/isaaclab_tasks/direct/iris_ma6/experiments/run_experiment.py \\
        --experiment a1_no_delay

    # Override seed and num_envs
    ./isaaclab.sh -p .../experiments/run_experiment.py \\
        --experiment a5_cbf_default --seed 42 --num_envs 2048
...
def _resume_from_checkpoint(checkpoint_path, trainer, agent):
    """Load checkpoint and set trainer to resume from correct timestep."""
    ...
    if match:
        ...
        if numbered:
            ...
            print(f"[INFO] Inferred resume timestep {resume_timestep}")
        else:
            raise ValueError(
...
def _log_dir_from_checkpoint(checkpoint_path):
    """Derive original log directory from checkpoint path."""
...
def _setup_logging(agent_cfg, exp_name, seed):
    ...
    log_root_path = os.path.join(
        "logs", "skrl",
        agent_cfg.get("agent", {}).get("experiment", {}).get("directory", "iris_ma6_experiments"),
    )
...
def main(env_cfg, agent_cfg: dict):
    ...
    env, possible_agents = _create_env(args_cli.task, env_cfg)
