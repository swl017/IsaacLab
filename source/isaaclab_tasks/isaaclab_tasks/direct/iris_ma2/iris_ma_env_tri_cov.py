...
from .triang_cov_reward_torch import (
    Intrinsics, 
    triangulation_covariance_multi_agent,
    triangulation_covariance_simple,
)
from .domain_randomization_config import (
    MeasurementNoiseCfg, 
    randomize_measurement_noise,
    apply_observation_noise,
    randomize_noise_on_reset,
)
...
class IrisMAEnvCfg(DirectMARLEnvCfg):
    ...
    observation_spaces = {
        "drone_0": 28, # [pos(3), rot(3), lin_vel(3), ang_vel(3), gimbal_yaw(1), gimbal_pitch(1), zoom(1)]
        "drone_1": 28,
    }
    ...
    measurement_noise: MeasurementNoiseCfg = MeasurementNoiseCfg(
        position_std_min=0.0,
        position_std_max=0.05,  # 5cm position noise
        orientation_std_min=0.0,
        orientation_std_max=0.01,  # ~0.5 degree orientation noise
        gimbal_yaw_std_min=0.0,
        gimbal_yaw_std_max=0.005,  # ~0.3 degree gimbal noise
        gimbal_pitch_std_min=0.0,
        gimbal_pitch_std_max=0.005,
        pixel_std_min=0.3,
        pixel_std_max=1.0,  # 0.3-1.0 pixel detection noise
        focal_length_std_ratio_min=0.0,
        focal_length_std_ratio_max=0.01,  # 1% focal length uncertainty
        principal_point_std_min=0.0,
        principal_point_std_max=2.0,  # 2 pixel principal point uncertainty
        enable_curriculum=True,
        curriculum_start_step=10000,
        curriculum_end_step=100000,
    )
    
    events = {
        "reset_noise": EventTermCfg(
            func=randomize_noise_on_reset,
            mode="reset",
        ),
    }
    ...
    triangulation_cov_reward_scale = 10.0  # Reward for minimizing uncertainty
    use_full_covariance: bool = False  # Use full covariance (slower) or simple (faster)
...
class IrisMAEnv(DirectMARLEnv):
    ...
    def __init__(self, cfg: IrisMAEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.measurement_noise = {
            'Sigma_twb': torch.zeros(self.num_envs, 3, 3, device=self.device),
            'Sigma_phiwb': torch.zeros(self.num_envs, 3, 3, device=self.device),
            'Sigma_alpha': torch.zeros(self.num_envs, 1, 1, device=self.device),
            'Sigma_beta': torch.zeros(self.num_envs, 1, 1, device=self.device),
            'Sigma_pix': torch.zeros(self.num_envs, 2, 2, device=self.device),
            'Sigma_K': torch.zeros(self.num_envs, 4, 4, device=self.device),
            'position_std': torch.zeros(self.num_envs, 3, device=self.device),
            'orientation_std': torch.zeros(self.num_envs, 3, device=self.device),
            'gimbal_yaw_std': torch.zeros(self.num_envs, device=self.device),
            'gimbal_pitch_std': torch.zeros(self.num_envs, device=self.device),
        }
        
        self.camera_intrinsics = {}
        for agent_id in self.cfg.possible_agents:
            cam_cfg = self.agent_camera_cfgs[agent_id]
            # Convert focal length from mm to pixels
            # sensor_width_mm = horizontal_aperture, sensor_width_px = width
            # fx = (focal_length_mm / sensor_width_mm) * sensor_width_px
            fx = (cam_cfg.spawn.focal_length / cam_cfg.spawn.horizontal_aperture) * cam_cfg.width
            fy = fx  # Assuming square pixels
            cx = cam_cfg.width / 2.0
            cy = cam_cfg.height / 2.0
            
            self.camera_intrinsics[agent_id] = Intrinsics(fx=fx, fy=fy, cx=cx, cy=cy)
        
        self.triangulation_trace = torch.zeros(self.num_envs, device=self.device)
        self.triangulation_cov = torch.zeros(self.num_envs, 3, 3, device=self.device)
        
        self._episode_sums["triangulation_cov"] = torch.zeros(
    ...
    def _get_observations(self) -> Dict[str, torch.Tensor]:
        ...
        # Compute triangulation uncertainty using TRUE or NOISY state
        robot_positions_true = [self._robots[aid].data.root_pos_w for aid in self.cfg.possible_agents]
        robot_quats_true = [self._robots[aid].data.root_state_w[:, 3:7] for aid in self.cfg.possible_agents]
        gimbal_yaws_true = [self._robots[aid].data.joint_pos[:, self.gimbal_joint_idx[aid]["yaw"]] 
                           for aid in self.cfg.possible_agents]
        gimbal_pitches_true = [self._robots[aid].data.joint_pos[:, self.gimbal_joint_idx[aid]["pitch"]] 
                              for aid in self.cfg.possible_agents]
        
        # Target position (true)
        target_pos_true = self.target.data.root_pos_w
        
        # Compute covariance using current noise levels
        intrinsics_list = [self.camera_intrinsics[aid] for aid in self.cfg.possible_agents]
        
        if self.cfg.use_full_covariance:
            # Full covariance with all noise sources
            self.triangulation_cov, self.triangulation_trace = triangulation_covariance_multi_agent(
                X_w=target_pos_true,
                robot_positions=robot_positions_true,
                robot_quats=robot_quats_true,
                gimbal_yaws=gimbal_yaws_true,
                gimbal_pitches=gimbal_pitches_true,
                camera_intrinsics=intrinsics_list,
                Sigma_pix=self.measurement_noise['Sigma_pix'],
                Sigma_twb=self.measurement_noise['Sigma_twb'],
                Sigma_phiwb=self.measurement_noise['Sigma_phiwb'],
                Sigma_alpha=self.measurement_noise['Sigma_alpha'],
                Sigma_beta=self.measurement_noise['Sigma_beta'],
                Sigma_K=None,  # Can enable if needed
                include_pose=True,
                include_gimbal=True,
                include_intrinsics=False,
            )
        else:
            # Simplified: only pixel noise (much faster)
            pixel_std = torch.sqrt(self.measurement_noise['Sigma_pix'][:, 0, 0])
            self.triangulation_cov, self.triangulation_trace = triangulation_covariance_simple(
                X_w=target_pos_true,
                robot_positions=robot_positions_true,
                robot_quats=robot_quats_true,
                gimbal_yaws=gimbal_yaws_true,
                gimbal_pitches=gimbal_pitches_true,
                camera_intrinsics=intrinsics_list,
                pixel_std=pixel_std.mean().item(),  # Average across batch
            )
        
        # Log-transform for better numerical properties
        log_trace = torch.log(self.triangulation_trace + 1e-6)

        observations = {}
        
        for agent_id in self.cfg.possible_agents:
            robot = self._robots[agent_id]
            robot_pos_noisy = apply_observation_noise(
                robot.data.root_pos_w,
                self.measurement_noise['position_std'],
                self.device
            )
            robot_vel = robot.data.root_lin_vel_w
            # Orientation
            roll, pitch, yaw = euler_xyz_from_quat(robot.data.root_state_w[:, 3:7])
            roll_noisy = roll + torch.randn_like(roll) * self.measurement_noise['orientation_std'][:, 0]
            pitch_noisy = pitch + torch.randn_like(pitch) * self.measurement_noise['orientation_std'][:, 1]
            yaw_noisy = yaw + torch.randn_like(yaw) * self.measurement_noise['orientation_std'][:, 2]
            quat_noisy = quat_from_euler_xyz(roll_noisy, pitch_noisy, yaw_noisy)
            
            # Gimbal angles with noise
            gimbal_pitch_noisy = apply_observation_noise(
                robot.data.joint_pos[:, self.gimbal_joint_idx[agent_id]["pitch"]:self.gimbal_joint_idx[agent_id]["pitch"]+1],
                self.measurement_noise['gimbal_pitch_std'].unsqueeze(-1),
                self.device
            )
            gimbal_yaw_noisy = apply_observation_noise(
                robot.data.joint_pos[:, self.gimbal_joint_idx[agent_id]["yaw"]:self.gimbal_joint_idx[agent_id]["yaw"]+1],
                self.measurement_noise['gimbal_yaw_std'].unsqueeze(-1),
                self.device
            )
