...
class GimbalStabilizer:
    ...
    def __init__(
        ...
        Args:
            cfg: Configuration with gimbal angle and rate limits.
            device: PyTorch device (cpu or cuda).
        """
        self.cfg = cfg
        self.device = device
        self.yaw_limits = list(cfg.yaw_limits)
        self.roll_limits = list(cfg.roll_limits)
        self.pitch_limits = list(cfg.pitch_limits)
    ...
    def integrate_gimbal_rates(
        ...
        Args:
            ...
            max_rate: maximum angular rate in rad/s (defaults to cfg.max_rate)
...
def create_gimbal_stabilizer(
    ...
    Args:
        ...
        max_rate_deg: Maximum angular rate in degrees/second

    Returns:
        GimbalStabilizer instance
    """
    cfg = GimbalStabilizerCfg(
        yaw_limits=(math.radians(yaw_limits_deg[0]), math.radians(yaw_limits_deg[1])),
        roll_limits=(math.radians(roll_limits_deg[0]), math.radians(roll_limits_deg[1])),
        pitch_limits=(math.radians(pitch_limits_deg[0]), math.radians(pitch_limits_deg[1])),
        max_rate=math.radians(max_rate_deg),
    )

    return GimbalStabilizer(cfg=cfg, device=device)
