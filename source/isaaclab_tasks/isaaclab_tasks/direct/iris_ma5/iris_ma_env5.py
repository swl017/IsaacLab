...
"""
Multi-Agent Iris Environment V5.
...
# Configuration
from isaaclab_tasks.direct.iris_ma5.iris_ma_env5_cfg import IrisMAEnvCfg

# BBox raycaster (from iris_ma5 or iris_ma3)
from isaaclab_tasks.direct.iris_ma5.bbox_raycaster import BBoxRayCaster, BBoxRayCasterCfg

# Triangulation utilities
from isaaclab_tasks.direct.iris_ma5.triangulation.triang_cov_reward_torch import (
...
from isaaclab_tasks.direct.iris_ma5.controller import PointMass, PointMassCfg, GimbalStabilizer, GimbalStabilizerCfg
from isaaclab_tasks.direct.iris_ma5.safety import SafetyManager, SafetyManagerCfg, CollisionDetectorCfg, TTCComputerCfg
from isaaclab_tasks.direct.iris_ma5.randomization import Randomizer, DistanceBasedFormationCfg
from isaaclab_tasks.direct.iris_ma5.visualization import CustomVisualization
from isaaclab_tasks.direct.iris_ma5.target_movement import TargetMovement
...
class IrisMAEnvV5(DirectMARLEnv):
    """Multi-Agent Iris Environment V5 using DelaySystemV2 architecture."""
    ...
    def __init__(self, cfg: IrisMAEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.progress_coord = self._linear_progress(curr.coordination_start_step, curr.coordination_end_step, current_step)
    ...
    def _compute_heuristic_reward(
        ...
        # Valid triangulation requires at least 2 agents detecting
        has_pair = (bbox_valid_mask.sum(dim=1) >= 2).float()
        return (proximity_reward + angle_reward) * has_pair
    ...
    def _build_initial_gt_states_for_reset(
        ...
        for agent_id in self.cfg.possible_agents:
            ...
            if reset_states and agent_id in reset_states:
                ...
                from isaaclab_tasks.direct.iris_ma5.delay_system_v2.derived_field_computers import (
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        for i, agent_id in enumerate(self.cfg.possible_agents):
            ...
            # Randomize zoom level between 1.0 and max_zoom_level
            random_zoom = 1.0 + torch.rand(len(env_ids), device=self.device) * (self.cfg.max_zoom_level - 1.0) * self.progress_tracking
