...
from isaaclab_assets import IRIS_GIMBAL_CFG  # isort: skip
...
class IrisGimbalEnvWindow(BaseEnvWindow):
    """Window manager for the Iris environment."""

    def __init__(self, env: IrisGimbalEnv, window_name: str = "IsaacLab"):
...
@configclass
class IrisGimbalEnvCfg(DirectRLEnvCfg):
    # env
    ...
    ui_window_class_type = IrisGimbalEnvWindow
    ...
    # robot
    robot: ArticulationCfg = IRIS_GIMBAL_CFG.replace(prim_path="/World/envs/env_.*/Robot")
...
class IrisGimbalEnv(DirectRLEnv):
    cfg: IrisGimbalEnvCfg

    def __init__(self, cfg: IrisGimbalEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        """
        Iris Gimbal Specific
        """
        roll, pitch, yaw = euler_xyz_from_quat(self._robot.data.root_state_w[:, 3:7])
        # rollpitch_exchange = quat_from_euler_xyz(
        #     pitch, -roll, yaw
        # )
        # self._robot.data.root_state_w[:, 3:7] = rollpitch_exchange
