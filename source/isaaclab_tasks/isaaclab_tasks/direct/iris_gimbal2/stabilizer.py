...
class DroneStabilizingController:
    ...
    def compute_control(
        ...
        roll = self.wrap_to_pi(roll)
        pitch = self.wrap_to_pi(pitch)
        ...
        # desired_thrust_per_mass = (9.82 + lin_acc_cmd_b_w[2]) / (
        #     z_w * quat_rotate_inverse(curr_quat_w, z_w))

        desired_thrust_per_mass = ((9.81 + lin_acc_cmd_w[:, 2]) / torch.cos(roll)) / torch.cos(pitch)
        # desired_thrust_per_mass = desired_thrust_per_mass.clip(0.0, 5.0)

        # desired_thrust_per_mass = torch.sqrt(
        #     (lin_acc_cmd_b_w[2] + 9.81)**2 * (1.0 + torch.tan(-roll)**2 + torch.tan(pitch)**2))

        # desired_thrust_per_mass = torch.sqrt(lin_acc_cmd_b_w[0]**2 + lin_acc_cmd_b_w[1]**2 + (lin_acc_cmd_b_w[2]-9.81)**2)
        ...
        desired_roll_pitch_sin = (1/desired_thrust_per_mass).view(-1, 1, 1) * (torch.linalg.inv(
        ...
        thrust = torch.zeros_like(curr_lin_vel_w)
        thrust[:, 2] = desired_thrust_per_mass * self.mass
        return thrust, ang_acc_cmd_b
    ...
    def reset(self, env_id):
        ...
        if self.cumul_lin_vel_error is not None:
            self.cumul_lin_vel_error[env_id, :] = torch.zeros_like(self.cumul_lin_vel_error)
        if self.last_lin_vel_error is not None:
            self.last_lin_vel_error[env_id, :] = torch.zeros_like(self.last_lin_vel_error)
        if self.last_ang_vel_error_b is not None:
            self.last_ang_vel_error_b[env_id, :] = torch.zeros_like(self.last_ang_vel_error_b)
