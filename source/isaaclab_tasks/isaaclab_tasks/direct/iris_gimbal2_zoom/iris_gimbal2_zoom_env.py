...
class IrisGimbal2ZoomEnvWindow(BaseEnvWindow):
    """Window manager for the Iris environment."""

    def __init__(self, env: IrisGimbal2ZoomEnv, window_name: str = "IsaacLab"):
...
@configclass
class IrisGimbal2ZoomEnvCfg(DirectRLEnvCfg):
    # env
    ...
    ui_window_class_type = IrisGimbal2ZoomEnvWindow
...
class IrisGimbal2ZoomEnv(DirectRLEnv):
    cfg: IrisGimbal2ZoomEnvCfg

    def __init__(self, cfg: IrisGimbal2ZoomEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Zoom state tracking (magnification level from x1 to x10)
        self.zoom_level = torch.ones(self.num_envs, device=self.device) * 1.0  # Start at x1 zoom
        self.base_focal_length = self.cfg.camera_cfg.spawn.focal_length  # Store original focal length
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # Process delta_zoom action (action index 6)
        delta_zoom = self._actions[:, 6] * self.step_dt * 5.0  # Scale delta zoom rate
        self.zoom_level += delta_zoom
        self.zoom_level = torch.clamp(self.zoom_level, 1.0, 10.0)  # Clamp to x1-x10 range

        # Update camera focal length based on zoom level (zoom = magnification)
        # Higher focal length = more zoomed in (narrower field of view)
        current_focal_length = self.base_focal_length * self.zoom_level
        self.camera_cfg_batch[:, 2] = current_focal_length
        
        # Update actual camera sensor focal length if camera exists
        if self._camera is not None:
            # Vectorized computation of intrinsic matrices using camera_cfg_batch
            width = self.camera_cfg_batch[:, 0]  # Camera width
            height = self.camera_cfg_batch[:, 1]  # Camera height
            focal_length_mm = self.camera_cfg_batch[:, 2]  # Updated focal length
            horizontal_aperture = self.camera_cfg_batch[:, 3]  # Horizontal aperture
            
            # Calculate focal lengths in pixels (vectorized)
            f_x = (width * focal_length_mm) / horizontal_aperture
            f_y = f_x  # Assuming square pixels
            c_x = width / 2.0
            c_y = height / 2.0
            
            # Create intrinsic matrices (vectorized)
            ...
            self._camera.set_intrinsic_matrices(intrinsic_matrices, focal_length=self.base_focal_length)
    ...
    def _apply_action(self):
        ...
        if DEBUG_DRAW and hasattr(self, 'draw_interface'):
            ...
            self.camera_frustrum.draw_frustrum(self.camera_pos_world, self.camera_quat_world, self.camera_cfg_batch, self.zoom_level, self.device)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # Reset zoom level to x1 for reset environments
