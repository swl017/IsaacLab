...
class DelaySystemCfg:
    ...
    # Detection sampler
    detection_fps_mean: float = 20.0  # Mean detection rate (Hz)
    detection_fps_std: float = 5.0  # Std deviation of detection rate (Hz)
    detection_latency_mean: float = 0.3  # 300ms mean processing latency
    detection_latency_std: float = 0.05  # 50ms std deviation
    detection_dropout_rate: float = 0.10  # 10% frame dropout

    # Ego communication (local, fast)
    ego_comm_latency: float = 0.005  # 5ms latency (nearly instant)

    # Inter-agent communication
    inter_agent_comm_rate_min: float = 20.0  # Min comm rate (Hz)
    inter_agent_comm_rate_max: float = 40.0  # Max comm rate (Hz) → 30 ± 10 Hz
    inter_agent_comm_latency_mean: float = 0.1  # 100ms mean latency
    inter_agent_comm_latency_std: float = 0.02  # 20ms std deviation
    inter_agent_comm_dropout_rate: float = 0.05  # 5% packet loss