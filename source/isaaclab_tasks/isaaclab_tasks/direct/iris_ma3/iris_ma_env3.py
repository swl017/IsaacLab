...
class IrisMAEnv(DirectMARLEnv):
    ...
    def _ensure_loom_buffers(self, N: int, A: int, device):
        """Ensure looming TTC buffers exist with correct dimensions."""
    ...
    def compute_looming_ttc_penalty_zoom_invariant(
        ...
        This method computes a Time-To-Collision penalty based on the visual looming effect,
        which is the rate of change of apparent size of the target in the image. The key
        innovation is making it zoom-invariant by using log(size/focal_length) instead of
        just log(size).

        Args:
            bbox_w_dn: Bbox width [N, A] (delayed+noisy, causal)
            bbox_h_dn: Bbox height [N, A] (delayed+noisy, causal)
            valid_dn: Valid mask [N, A] (delayed+noisy, causal)
            fx_delayed: Focal length x [N, A] (delayed, noiseless)
            fy_delayed: Focal length y [N, A] (delayed, noiseless)
            dt: Time step (scalar)
            horizon_sec: Time horizon for TTC computation
            ema_alpha_s: EMA coefficient for log-size
            ema_alpha_f: EMA coefficient for log-focal
            deriv_clip: Clipping value for derivative
            eps: Small epsilon for numerical stability
            stale_half_life: Half-life for staleness decay
            zoom_gate_k: Gate aggressiveness for zoom motion
            aggregate: Aggregation method ("max" or "mean")

        Returns:
            team_phi: Aggregated TTC penalty [N]
            phi: Per-agent TTC penalty [N, A]
            tau: Time-to-collision [N, A]
        """
        N, A = bbox_w_dn.shape
        ...
        # Aspect gate (optional) - currently not used but kept for reference
    ...
    def _get_rewards(self) -> Dict[str, torch.Tensor]:
        ...
        # Collect delayed+noisy bbox data for all agents
        bbox_w_list = []
        bbox_h_list = []
        bbox_valid_list = []
        fx_list = []
        ...
        for agent_id in self.cfg.possible_agents:
            ...
            delayed_state = self.state_manager.get_delayed_states(agent_id)  # For noiseless intrinsics

            # Extract bbox width and height from delayed+noisy state
            bbox_w = noisy_state.data.bboxes_2d[:, 0, 2]  # [N], width
            bbox_h = noisy_state.data.bboxes_2d[:, 0, 3]  # [N], height
            bbox_valid = noisy_state.data.bboxes_2d_valid_mask[:, 0, 0]  # [N], bool

            # Extract focal lengths from delayed (noiseless) intrinsics
            fx = delayed_state.data.camera_intrinsics[:, 0]  # [N], fx
            fy = delayed_state.data.camera_intrinsics[:, 1]  # [N], fy

            bbox_w_list.append(bbox_w)
        ...
        # Stack to [N, A] format
        bbox_w_all = torch.stack(bbox_w_list, dim=1)  # [N, A]
        bbox_h_all = torch.stack(bbox_h_list, dim=1)  # [N, A]
        bbox_valid_all = torch.stack(bbox_valid_list, dim=1)  # [N, A]
        fx_all = torch.stack(fx_list, dim=1)  # [N, A]
        fy_all = torch.stack(fy_list, dim=1)  # [N, A]

        # Compute looming TTC penalty
        ttc_horizon = self.cfg.ttc_horizon if hasattr(self.cfg, 'ttc_horizon') else 6.0
        _, ttc_penalty_cam_to_target, _ = self.compute_looming_ttc_penalty_zoom_invariant(
            bbox_w_dn=bbox_w_all,
            bbox_h_dn=bbox_h_all,
            valid_dn=bbox_valid_all,
            fx_delayed=fx_all,
            fy_delayed=fy_all,
            dt=self.step_dt,
            horizon_sec=ttc_horizon,
            ema_alpha_s=0.6,
            ema_alpha_f=0.6,
            deriv_clip=0.5,
            eps=1e-6,
        ...
        for i, agent_id in enumerate(self.cfg.possible_agents):
            ...
            # Time-to-collision penalty using looming TTC
            ttc_penalty = ttc_penalty_cam_to_target[:, i]  # Extract this agent's TTC penalty
