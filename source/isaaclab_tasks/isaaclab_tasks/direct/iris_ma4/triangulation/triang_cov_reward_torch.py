...
def midpoint_method_batched(
    ...
    # Check for singular matrices per environment BEFORE solving
    rank = torch.linalg.matrix_rank(A)  # [N, T]
    is_singular = (rank < 3) | insufficient.unsqueeze(-1)  # [N, T]

    # Use lstsq instead of solve - it's more robust and handles singular matrices gracefully
    # lstsq returns least-squares solution even for rank-deficient matrices
    # This avoids the issue where one singular env causes ALL envs to fail
    try:
        # lstsq returns (solution, residuals, rank, singular_values)
        # For batched input A [N, T, 3, 3] and b [N, T, 3, 1], we need to reshape
        A_flat = A.reshape(N * T, 3, 3)  # [N*T, 3, 3]
        b_flat = b.reshape(N * T, 3, 1)  # [N*T, 3, 1]

        # Use lstsq which is robust to singular matrices
        X_mid_flat = torch.linalg.lstsq(A_flat, b_flat).solution  # [N*T, 3, 1]
        X_mid = X_mid_flat.reshape(N, T, 3)  # [N, T, 3]

        # Check for NaN/Inf per-environment (can happen with very ill-conditioned matrices)
        has_nan_inf = torch.isnan(X_mid).any(dim=-1) | torch.isinf(X_mid).any(dim=-1)  # [N, T]
        is_triangulation_successful = ~has_nan_inf  # [N, T] - per-environment success flag

    except Exception:
        # Complete fallback if lstsq itself fails (shouldn't happen normally)
        X_mid = mean_pts.clone()  # [N, T, 3]
        is_triangulation_successful = torch.zeros(N, T, dtype=torch.bool, device=device)

    # Use torch.where for per-environment fallback (not global .any())
    # Replace singular or NaN/Inf results with mean_pts
    needs_fallback = is_singular | ~is_triangulation_successful  # [N, T]
    X_mid = torch.where(needs_fallback.unsqueeze(-1), mean_pts, X_mid)
    ...
    is_valid = ~(is_singular | is_behind_all) & is_triangulation_successful  # [N, T]
