...
Usage:
    ...
    ./isaaclab.sh -p source/isaaclab_tasks/isaaclab_tasks/direct/iris_ma4/experiments/run_experiment.py \\
...
def _setup_logging(agent_cfg, exp_name, seed):
    ...
    log_root_path = os.path.join(
        "logs", "skrl",
        agent_cfg.get("agent", {}).get("experiment", {}).get("directory", "iris_ma4_experiments"),
    )
...
def _train_mlp(env, possible_agents, agent_cfg, exp_cfg, seed, log_dir, env_cfg, checkpoint_path=None):
    ...
    from isaaclab_tasks.direct.iris_ma4.experiments.models.mappo_mlp_agent import (
        MAPPO_MLP,
