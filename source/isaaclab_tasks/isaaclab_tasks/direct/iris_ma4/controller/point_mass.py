...
class PointMass:
    def __init__(
        ...
        """Initialize point mass controller.

        Args:
            cfg: Controller configuration with gains and limits.
            mass: Robot mass in kg.
            num_envs: Number of parallel environments.
            device: PyTorch device (e.g., 'cuda:0').
            weight: Robot weight in N. If None, computed as mass * 9.81.
            disable_gravity: Whether gravity compensation is disabled.
        """
        self.cfg = cfg
        self.mass = mass
        ...
        # Initialize gains from config
        self.default_gains = {
            # Position control
            "kp_x": torch.tensor([cfg.kp_x], device=device).expand(num_envs),
            "ki_x": torch.tensor([cfg.ki_x], device=device).expand(num_envs),
            "kd_x": torch.tensor([cfg.kd_x], device=device).expand(num_envs),
            "kp_y": torch.tensor([cfg.kp_y], device=device).expand(num_envs),
            "kd_y": torch.tensor([cfg.kd_y], device=device).expand(num_envs),
            "kp_z": torch.tensor([cfg.kp_z], device=device).expand(num_envs),
            # Attitude control
            "kp_att": torch.tensor([cfg.kp_att], device=device).expand(num_envs),
            "kd_att": torch.tensor([cfg.kd_att], device=device).expand(num_envs),
            "kp_yaw": torch.tensor([cfg.kp_yaw], device=device).expand(num_envs),
            "kd_yaw": torch.tensor([cfg.kd_yaw], device=device).expand(num_envs),
        }

        # Maximum moment limits from config
        self.max_moment = torch.tensor(cfg.max_moment, device=device).view(1, 3).expand(num_envs, 3)
        ...
        # Thrust limits from config
        self._thrust_scale_min = cfg.thrust_scale_min
        self._thrust_scale_max = cfg.thrust_scale_max
    ...
    def compute_control(
        ...
        force[:, 2] = torch.clamp(force[:, 2], min=-self.weight * self._thrust_scale_min, max=self.weight * self._thrust_scale_max)
